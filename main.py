...
def load_args(default_config=None):
    ...
    # -- dataset config
    parser.add_argument('--dataset', default='Dataset01', help='dataset selection')
    parser.add_argument('--num-classes', type=int, default=25, help='Number of classes')
    parser.add_argument('--modality', default='video', help='choose the modality')
    # -- directory
    parser.add_argument('--data-dir', default='./datasets/Dataset_01_ROIs', help='Loaded data directory')
    parser.add_argument('--label-path', type=str, default='./labels/25WordsSortedList.txt', help='Path to txt file with labels')
    parser.add_argument('--annonation-direc', default=None, help='Loaded data directory')
    # -- model config
    parser.add_argument('--backbone-type', type=str, default='efficientnet', help='Architecture used for backbone')
...
def extract_feats(model):
    ...
    data = preprocessing_func(np.load(args.mouth_patch_path)['data']) 
